...
import { Settings, Lock, Unlock, CheckCircle, Copy, Users, Flag, Save, AlertTriangle, ChevronLeft, UserCircle, ChevronDown, Activity, Timer } from "lucide-react";
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-4xl md:text-5xl font-display font-black text-white italic uppercase tracking-tighter leading-none" data-testid="text-admin-title">
              {selectedRace ? (
                <>
                  <span className="text-primary">{lobby?.name}</span> • {selectedRace.name} <span className="text-primary">Control</span>
                </>
              ) : (
                <>Race <span className="text-primary">Selection</span></>
              )}
            </h1>
          </div>
        </div>
